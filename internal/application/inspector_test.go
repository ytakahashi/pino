...
func inspected(t *testing.T, root domain.Node, pointer string) InspectorInfo {
	t.Helper()

	app := session(t, root)

	cursor, err := domain.ParsePointer(pointer)
	if err != nil {
		t.Fatalf("ParsePointer(%q): %v", pointer, err)
	}

	app.view.Cursor = cursor
...
func TestInspectorWithoutDocument(t *testing.T) {
	t.Parallel()
