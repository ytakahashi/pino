...
func TestLayoutForChoosesAPlacementFromTheAvailableSpace(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"the JSON view on a narrow terminal": {
			width: 60, height: 12, view: application.ViewJSON,
			want: layout{BodyWidth: 60, BodyHeight: 11, Inspector: placeNone},
		},
		...
		"the tree view at the smallest size": {
			width: 60, height: 12, view: application.ViewTree,
			want: layout{
				BodyWidth: 60, BodyHeight: 5,
				Inspector: placeBelow, InspectorHeight: 6,
			},
		},
		...
		"one row too short": {
			width: 120, height: 11, view: application.ViewJSON,
			want: layout{TooSmall: true, BodyWidth: 120, BodyHeight: 10, Inspector: placeNone},
		},

		// Wide enough and still too short, which is the case a check on the
		// width alone would let through.
		"wide but too short": {
			width: 120, height: 11, view: application.ViewTree,
			want: layout{
				TooSmall: true, BodyWidth: 87, BodyHeight: 10,
				Inspector: placeSide, InspectorWidth: 32,
			},
		},
...
func TestLayoutForPrioritizesTheDocumentWhilePrompting(t *testing.T) {
	...
	for _, view := range views {
		for _, width := range []int{59, 60, 99, 100} {
			for _, height := range []int{11, 12, 40} {
				...
				if view == application.ViewTree && width == minWidth && height == minHeight && asked.BodyHeight != 7 {
					t.Errorf("layoutFor(60, 12, Tree) leaves %d body rows with a notice, want 7", asked.BodyHeight)
				}
